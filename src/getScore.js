const getRefsCount = item =>
  (item.refs && item.refs.filter(r => r.url).length) || 0;
const getVariantsCount = item =>
  (item.variants && item.variants.split("\n").filter(r => !!r.trim()).length) ||
  0;
...
const getScore = (collection, item) => {
  ...
  if (collection === "glossaire") {
    ...
    score += Math.min(
      30,
      item.abbrs && item.abbrs.split && item.abbrs.split("\n").length * 10,
      0
    );
  }
  if (collection === "ccns") {
    score +=
      (item.groups &&
        item.groups.filter &&
        item.groups.filter(group => group.selection && group.selection.length)
          .length * 8) ||
      0;
  }
